...
**근사 구간**일 뿐이다. 리포트에 %를 표시할 때는 반드시 이 사실을 함께 명시한다.

```text
confirmed         → 90–100% (근사)
strongly-inferred → 60–89%  (근사)
weakly-inferred   → 20–59%  (근사)
not-observed      → 0%      (근사)
```

구간 안에서의 구체적인 숫자(예: "72%")는 등급 내에서의 상대적 위치를 나타내는 표현적
장치이지, 계산된 값이 아니다. 등급 자체보다 숫자를 더 신뢰하지 않도록 리포트 본문에는 항상
등급 이름과 % 근사값을 함께 적는다(% 단독 표기 금지).
...
- **명시적 호출**: 프롬프트 원문에 `/plugin:skill` 형태의 문자열이 그대로 있으면, 그
  문자열 자체가 근거 문구다 (`confirmed`).
- **키워드 일치**: 프롬프트에 등장하는 단어/구가 후보 Skill의 `description`에 있는 트리거
  키워드(예: "차트", "배포", "리뷰")와 겹치고, 실제로 그 Skill의 파일 접근 또는 유사 절차
  수행이 관찰되면, 그 단어/구를 근거 문구로 표시한다 (`strongly-inferred` 이하로 판정 —
  키워드 일치만으로는 `confirmed`를 주지 않는다).
