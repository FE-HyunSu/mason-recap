...
```text
confirmed         → 90–100% (참고용)
strongly-inferred → 60–89%  (참고용)
weakly-inferred   → 20–59%  (참고용)
not-observed      → 알수없음 (수치로 표기하지 않음)
```

**not-observed는 "0%"라는 숫자를 쓰지 않고 "알수없음"으로만 표기한다.** "0%"는 마치
...
등급 이름과 % 참고용 수치를 함께 적는다(% 단독 표기 금지, not-observed는 %를 아예 쓰지
않고 "알수없음"만 적는다).
...
- **셀 안에서 줄을 바꿔야 하면 실제 줄바꿈 대신 `<br>`을 쓴다**: 마크다운 표는 셀 내부에
  raw newline이 들어가면 표 구조 자체가 깨진다. 한 트리거에 대해 근거가 여러 개면
  `<br>`로 구분해 같은 셀 안에 짧게 나열한다.
